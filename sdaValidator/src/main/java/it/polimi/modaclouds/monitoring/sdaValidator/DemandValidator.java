...
public class DemandValidator {
			   ...
			   if(startCollectSDAResult & !stop){
				   ...
				   if(checkIntervalCompleted()){
					   ...
					   if(line.contains("estimationci")){
						   splitted=line.split(" ");
						   String datum=splitted[splitted.length-1].substring(1, splitted[splitted.length-1].length()-1);
						   Float toAdd=Float.parseFloat(datum.split(":")[1]);
						   
						   if(datum.split(":")[0].equals("mic-frontend-mon-saveAnswers")){
							   saveDem=toAdd;
						   }else if(datum.split(":")[0].equals("mic-frontend-mon-register")){
							   regDem=toAdd;
						   }else if(datum.split(":")[0].equals("mic-frontend-mon-answerQuestions")){
							   answDem=toAdd;
						   }
					   }
					   ...
					   if(line.contains("AvarageResponseTime")){
						   splitted=line.split(" ");
						   Float toAdd=Float.parseFloat(splitted[2].split("e")[0]);
	
						   
						   if(splitted[0].equals("mic-frontend-mon-saveAnswers")){
							   saveRT.add(toAdd);
						   }else if(splitted[0].equals("mic-frontend-mon-register")){
							   regRT.add(toAdd);
	
						   }else if(splitted[0].equals("mic-frontend-mon-answerQuestions")){
							   answRT.add(toAdd);
	...
	private static String validate(){
		...
		*/
		...
		return U+
				","+saveResult[0]+","+saveResult[1]+","+saveResult[2]+","+saveResult[3]+","+
				regResult[0]+","+regResult[1]+","+regResult[2]+","+regResult[3]+","+
				answResult[0]+","+answResult[1]+","+answResult[2]+","+answResult[3];
